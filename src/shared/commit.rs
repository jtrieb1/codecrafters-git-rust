...
impl CommitAuthor {
    ...
    pub fn from_env() -> Self {
        let name = std::env::var("GIT_AUTHOR_NAME").unwrap_or_else(|_| "Author Name".to_string());
        let email =
            std::env::var("GIT_AUTHOR_EMAIL").unwrap_or_else(|_| "author@example.com".to_string());
        ...
        CommitAuthor::new(name, email, timestamp, timezone)
    }
    ...
    pub fn from_str(s: &str) -> Result<Self, String> {
        ...
        let prefix_end = s
            .find(' ')
            .ok_or("Invalid author/committer line: missing prefix")?;
        ...
        let name_end = data_line
            .find('<')
            .ok_or("Invalid author/committer line: missing email")?;
        let name = data_line[..name_end].trim().to_string();

        // The email is between the '<' and '>' characters
        let email_start = name_end + 1;
        let email_end = data_line[email_start..]
            .find('>')
            .ok_or("Invalid author/committer line: malformed email")?
            + email_start;
        ...
        let timestamp_str = rest_parts
            .next()
            .ok_or("Invalid author/committer line: missing timestamp")?;
        let timezone = rest_parts
            .next()
            .ok_or("Invalid author/committer line: missing timezone")?
            .to_string();
...
impl Commit {
    ...
    pub fn from_input(message: String, parents: Vec<String>, tree: String) -> Result<Self, String> {
        let tree_hash = hash_from_string(&tree);
        let tree_obj = Object::try_from_hash(&tree_hash)
            .map_err(|e| format!("Tree object not found: {}", e))?;
        ...
        for parent in parents {
            ...
            let parent_obj = Object::try_from_hash(&parent_hash)
                .map_err(|e| format!("Parent commit object not found: {}", e))?;
        ...
        Ok(Commit {
            tree: tree_hash,
            parents: parent_hashes.iter().map(|h| h.to_vec()).collect(),
            author: CommitAuthor::from_env(),
            committer: CommitAuthor::from_env(),
            message,
        })
    }
    ...
    fn parse_tree_line(line: &str) -> Result<Vec<u8>, String> {
        let tree_hash_str = line
            .strip_prefix("tree ")
            .ok_or("Invalid tree line")?
            .trim();
        Ok(hash_from_string(tree_hash_str))
    }

    fn parse_parent_line(line: &str) -> Result<Vec<u8>, String> {
        let parent_hash_str = line
            .strip_prefix("parent ")
            .ok_or("Invalid parent line")?
            .trim();
    ...
    pub fn from_content(content: &[u8]) -> Result<Self, String> {
        ...
        for line in content_str.lines() {
            ...
            if line.starts_with("parent ") {
                parents.push(Self::parse_parent_line(line)?);
                continue;
            }
            ...
            if line.trim().is_empty() {
                let message = content_str
                    .splitn(2, "\n\n")
                    .nth(1)
                    .unwrap_or("")
                    .to_string();
                // For some reason, the test commit objects have a trailing newline in the message
    ...
    pub fn pretty_print(&self) -> String {
        ...
        output.push_str(&format!("tree {}\n", hash_to_string(&self.tree)));
        for parent in &self.parents {
            output.push_str(&format!("parent {}\n", hash_to_string(parent)));
        }
    ...
    pub fn try_get_tree(&self) -> Result<Tree, String> {
        let tree_obj = Object::try_from_hash(&self.tree)
            .map_err(|e| format!("Tree object not found: {}", e))?;
        if *tree_obj.object_type() != ObjectType::Tree {
            return Err(format!(
    ...
    fn from(value: &Commit) -> Self {
        ...
        content.extend_from_slice(
            format!(
                ...
                value.author.name,
                value.author.email,
                value.author.timestamp,
                value.author.timezone
            )
            .as_bytes(),
        );
        content.extend_from_slice(
            format!(
                ...
                value.committer.timestamp,
                value.committer.timezone
            )
            .as_bytes(),
        );
