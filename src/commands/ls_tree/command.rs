...
pub fn ls_tree(input: LsTreeInput) -> Result<String, LsTreeError> {
    let sha_bytes = hash_from_string(&input.sha);
    let object = Object::try_from_hash(&sha_bytes)
        .map_err(|_| LsTreeError::ObjectNotFound(input.sha.clone()))?;
    if let Ok(tree) = Tree::try_from(&object) {
        if input.name_only {
            return Ok(tree.print_names());
        } else {
            return Ok(tree.print_entries());
        }
    } else {
